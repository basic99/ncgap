<?php 
/**
 * Page shows user defined AOI
 * 
 * If $_SESSION['username'] is not visitor, get saved AOI from table aoi and 
 * display selections in a form.
 * 
 * @package ncgap
 */
session_start();
		...
		$.ajax({
			...
			success: function(data){
				if(data.success){
					...
					document.forms.fm2.action = zipfile;
...
<?php
$query = "select name, description from aoi where username = '{$user}'";
$result = pg_query($query);
...
<!-- this form is used to get shapefile download -->
<form target="_self" method="GET" name="fm2">
</form>
<br />
